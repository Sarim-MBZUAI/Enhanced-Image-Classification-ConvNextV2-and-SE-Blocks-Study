...
class ModifiedBlock(nn.Module):
    def __init__(self, original_blocks, att_block):
        ...
        self.first_blocks = nn.Sequential(*list(original_blocks)[:3]) #: 2 for timm-cnextL, 5 for tv-cnextL
        # Custom SE block
        self.att_block = att_block
        # The remaining layers of the original block
        self.last_blocks = nn.Sequential(*list(original_blocks)[3:])
...
path = os.path.join('/home/thao.nguyen/CV703/Assignment_01/Graph+code/Modified/', f"task2_AttbtwS34.pth") #CHANGE
...
log_dir = '/home/thao.nguyen/CV703/Assignment_01/Graph+code/Modified/'  # Change to your desired log directory
if not os.path.exists(log_dir):
    os.makedirs(log_dir)
log_filename = datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + 'task2_AttbtwS34.log'
